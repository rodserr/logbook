...
.green_epl <- "#04b46c"
.purple_epl <- "#3f043f"
thematic_shiny(font = "auto", qualitative = c(.purple_epl, .green_epl))
...
ui <- navbarPage(
    theme = bs_theme(primary = .purple_epl, success = .green_epl, 
    ...
    tabPanel(
        ...
        fluidRow(
            column(
                ...
                DT::dataTableOutput('standings') %>% withSpinner(color = .green_epl)
            ),
            column(
                6,
                h3('Total xG vs xGA by Team'),
                htmltools::includeMarkdown('www/understat.md'),
                br(),
                plotOutput('total_xG', height = '550px') %>% withSpinner(color = .green_epl)
            )
        )
    )
)
